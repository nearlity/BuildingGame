...
namespace UnityChan
{
    public class SpringManager : MonoBehaviour
    {
        ...
        private void UpdateStiffnessForce(float baseValue, AnimationCurve curve)
        {
            ...
            for (int i = 0; i < springBones.Length; i++)
            {
                var springBone = springBones[i];
                if (!springBone.isUseEachBoneForceSettings)
                {
                    var scale = curve.Evaluate(start + i * step);
                    springBone.stiffnessForce = baseValue * scale;
                }
            }
        }
        ...
        private void UpdateDragForce(float baseValue, AnimationCurve curve)
        {
            ...
            for (int i = 0; i < springBones.Length; i++)
            {
                ...
                {
                    var scale = curve.Evaluate(start + i * step);
                    springBone.dragForce = baseValue * scale;
                }
            }
        }
    }
